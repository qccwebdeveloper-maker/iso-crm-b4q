...
export default function AuditorAuditReportList() {
    ...
    <Layout title="Audit Reports">
      <div style={{ padding: '24px 28px', maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 24 }}>
          ...
          <div>
            <h2 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: 'var(--gray-800)' }}>Assigned Audit Reports</h2>
            <p style={{ margin: 0, fontSize: 12, color: 'var(--gray-500)' }}>Reports assigned to you for auditing</p>
          </div>
        </div>
        ...
        <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 10, padding: '12px 16px', marginBottom: 20, fontSize: 13, color: '#1d4ed8' }}>
          <strong>Your access:</strong> You can fill Steps 1–6. Step 7 (Certificate & Review) is managed by admin only.
        </div>
        ...
        <div style={{ background: 'white', borderRadius: 12, border: '1px solid #f1f5f9', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
          {loading ? (
            ...
            <div style={{ padding: '60px 20px', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--gray-500)', marginBottom: 4 }}>No reports assigned yet</div>
              <div style={{ fontSize: 13, color: 'var(--gray-400)' }}>Reports assigned to you will appear here</div>
            </div>
          ) : (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: '#f8fafc', borderBottom: '2px solid #f1f5f9' }}>
                    {['Ref No.', 'Organization', 'Client', 'Status', 'Created', 'Action'].map(h => (
                      ...
                      <tr key={r._id} style={{ borderBottom: '1px solid #f8fafc', background: i % 2 === 0 ? 'white' : '#fafafa' }}>
                        ...
                        <td style={{ padding: '12px 16px', fontSize: 12 }}>
                          {r.client ? (
                            ...
                            <span style={{ color: 'var(--gray-300)', fontStyle: 'italic', fontSize: 11 }}>—</span>
                          )}
                        </td>
                        ...
                        <td style={{ padding: '12px 16px' }}>
                          <button
                            onClick={() => navigate(`/auditor/audit-reports/${r._id}`)}
