...
// Client can only see Steps 1 and 2
const CLIENT_STEPS = STEPS.filter(s => s.id <= 2);

export default function ClientAuditReportDetail() {
  const { id }   = useParams();
  ...
  const currentIdx   = CLIENT_STEPS.findIndex(s => s.id === step);
  const isFirst      = currentIdx === 0;
  const isLast       = currentIdx === CLIENT_STEPS.length - 1;
      ...
      <div style={{ padding: '24px 28px', maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <button
              onClick={() => navigate('/client/audit-reports')}
            ...
            <div>
              ...
              <p style={{ margin: 0, fontSize: 12, color: 'var(--gray-500)' }}>Fill in your organization information</p>
            </div>
          </div>
          <button
            onClick={handleSave}
        ...
        <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 10, padding: '10px 16px', marginBottom: 16, fontSize: 12, color: '#1d4ed8' }}>
          You can fill in <strong>Organization Information</strong> and <strong>Standards & Employees</strong> sections only. Other sections are managed by the audit team.
        </div>

        {/* Step Indicator (only 2 steps) */}
        <StepIndicator steps={CLIENT_STEPS} currentStep={step} onStepClick={setStep} />
        ...
        <div style={{ background: 'white', borderRadius: 12, border: '1px solid #f1f5f9', padding: 28, boxShadow: '0 1px 3px rgba(0,0,0,0.06)', minHeight: 400 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20, paddingBottom: 16, borderBottom: '2px solid #f8fafc' }}>
            {React.createElement(CLIENT_STEPS[currentIdx].icon, { size: 18, color: 'var(--primary)' })}
            <span style={{ fontSize: 16, fontWeight: 700, color: 'var(--gray-800)' }}>
              Step {currentIdx + 1}: {CLIENT_STEPS[currentIdx].label}
            </span>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 20 }}>
          <button
            onClick={() => !isFirst && setStep(CLIENT_STEPS[currentIdx - 1].id)}
            disabled={isFirst}
          ...
          <span style={{ fontSize: 12, color: 'var(--gray-400)' }}>Step {currentIdx + 1} of {CLIENT_STEPS.length}</span>
          {!isLast
            ? <button
                onClick={() => setStep(CLIENT_STEPS[currentIdx + 1].id)}
