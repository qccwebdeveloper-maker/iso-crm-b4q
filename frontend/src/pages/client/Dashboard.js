...
import { FileText, Clock, Award, XCircle, Plus, ChevronRight, ArrowRight, Star, CheckCircle, AlertCircle, Send, ClipboardCheck, Phone, Edit, Eye } from 'lucide-react';

const STAGES = ['submitted','under_review','audit_stage1','audit_stage2','approved','certified'];
const STAGE_INFO = {
  draft:        { label:'Draft',         icon:'📝', color:'var(--gray-400)',  desc:'Application saved as draft' },
  submitted:    { label:'Submitted',     icon:'📤', color:'var(--blue)',      desc:'Received — awaiting review' },
  under_review: { label:'Under Review',  icon:'🔍', color:'var(--amber)',     desc:'Admin reviewing your application' },
  audit_stage1: { label:'Audit Stage 1', icon:'📋', color:'var(--purple)',    desc:'Document audit in progress' },
  audit_stage2: { label:'Audit Stage 2', icon:'🏭', color:'#7e22ce',          desc:'On-site audit in progress' },
  approved:     { label:'Approved',      icon:'✅', color:'var(--teal)',      desc:'Approved — certificate being prepared' },
  certified:    { label:'Certified',     icon:'🏆', color:'var(--green)',     desc:'Certification complete!' },
};
...
export default function ClientDashboard() {
  ...
  const quickActions = [
    { l:'New Application', d:'Start ISO certification', icon:'📋', to:'/client/applications/new', c:'var(--primary)' },
    { l:'My Documents',    d:'Upload required forms',   icon:'📁', to:'/client/documents',        c:'var(--blue)'   },
    { l:'Certificates',    d:'Download certificates',   icon:'🏆', to:'/client/certificates',     c:'var(--amber)'  },
    { l:'Send Feedback',   d:'Rate your experience',    icon:'💬', onClick:()=>setFbModal(true),  c:'var(--teal)'   },
  ];
  ...
  return (
    <Layout title="Dashboard">
      ...
      <div className="client-quick-grid">
        {quickActions.map((a,i)=>(
          <div key={i} className="quick-action" onClick={a.onClick||(() => navigate(a.to))}>
            <div className="qa-icon" style={{background:`${a.c}18`,border:`1px solid ${a.c}30`}}>{a.icon}</div>
      ...
      {/* ── Certification Progress — full-width row ── */}
      <div className="card" style={{marginBottom:18}}>
        ...
        {latestApp && stageInfo ? (
          <div style={{padding:'16px 20px'}}>
            {/* Top row: status banner + progress % + payment */}
            <div style={{display:'flex',alignItems:'center',gap:14,marginBottom:16,flexWrap:'wrap'}}>
              <div style={{display:'flex',alignItems:'center',gap:10,padding:'10px 16px',background:`${stageInfo.color}12`,borderRadius:9,border:`1.5px solid ${stageInfo.color}30`,flex:'0 0 auto'}}>
                <span style={{fontSize:24}}>{stageInfo.icon}</span>
                <div>
              ...
              {latestApp.paymentStatus && (
                <div style={{padding:'10px 14px',background:'var(--amber-50)',borderRadius:8,border:'1px solid var(--amber-100)',flex:'0 0 auto'}}>
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:7}}>
                    <span className={`badge ${latestApp.paymentStatus==='received'?'bdg-approved':'bdg-submitted'}`} style={{fontSize:10}}>
                      {latestApp.paymentStatus==='received'?'✓ Received':latestApp.paymentStatus==='partially_received'?'◐ Partial':'⏳ Pending'}
                    </span>
              ...
              <tbody>
                {recentApps.map(app=>(
                  <tr key={app._id}>
                    ...
                    <td>
                      <span className="badge bdg-info">{app.isoStandard}</span>
                    </td>
                    <td>
                    ...
                    <td>
                      <span className={`badge ${app.paymentStatus==='received'?'bdg-approved':app.paymentStatus==='partially_received'?'bdg-audit_stage2':'bdg-submitted'}`}>
                        {app.paymentStatus==='received'?'✓ Paid':app.paymentStatus==='partially_received'?'◐ Partial':'⏳ Pending'}
                      </span>
                    </td>
                    <td>
      ...
      <div className="client-dash-grid">
        ...
        <div className="card" style={{marginBottom:0}}>
          ...
          {latestApp ? (
            [
              {role:'Auditor', emoji:'🔍', person:latestApp.assignedAuditor,  report:latestApp.auditReport,  color:'var(--primary)', bg:'var(--primary-50)'},
              {role:'Reviewer',emoji:'✅', person:latestApp.assignedReviewer, report:latestApp.reviewReport, color:'#7c3aed',        bg:'#f5f3ff'},
            ].map(({role,emoji,person,report,color,bg},idx)=>(
        ...
        <div className="card" style={{marginBottom:0}}>
          ...
          <div>
            {[
              {icon:'📄',text:'Upload signed agreement', sub:'Required document',       urgent:true,  action:()=>navigate('/client/documents')},
              {icon:'📝',text:'Complete org profile',    sub:'Fill in company details',  urgent:false, action:()=>navigate('/client/applications')},
              {icon:'🏆',text:'Download certificate',    sub:'Available after approval', urgent:false, action:()=>navigate('/client/certificates')},
              {icon:'💬',text:'Send feedback',           sub:'Rate your experience',     urgent:false, action:()=>setFbModal(true)},
            ].map((item,i,arr)=>(
              <div key={i}
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{fontSize:11,color:item.urgent?'var(--red)':'var(--gray-400)',marginTop:2,fontWeight:item.urgent?700:400}}>
      ...
      {editModal && (
        <div className="modal-bg" onClick={()=>setEditModal(null)}>
          <div className="modal-box" onClick={e=>e.stopPropagation()}>
            <div className="modal-head"><div className="modal-title"><Edit size={14} style={{color:'var(--primary)',marginRight:7,verticalAlign:'middle'}}/>Edit — {editModal.applicationId}</div><button className="modal-close" onClick={()=>setEditModal(null)}>✕</button></div>
      ...
      {fbModal && (
        <div className="modal-bg" onClick={()=>setFbModal(false)}>
          <div className="modal-box" onClick={e=>e.stopPropagation()}>
            <div className="modal-head"><div className="modal-title">Send Feedback to Admin</div><button className="modal-close" onClick={()=>setFbModal(false)}>✕</button></div>
            <div className="modal-body">
            ...
            <div className="modal-foot">
              ...
              <button className="btn btn-primary" onClick={submitFeedback} disabled={submitting}>{submitting?'Sending…':<><Send size={13}/> Send to Admin</>}</button>
            </div>
          </div>
        </div>
      )}
